      ...
      <div className="container mx-auto max-w-4xl">
        <div className="flex h-14 items-center justify-between px-6 liquid-glass-header rounded-full">
          <Link href="/3D-architecture-visualization-studio" className="flex items-center gap-1.5">
            ...
            <span className="sr-only">Kyndl ArchViz</span>
          </Link>
          ...
          <nav className="hidden items-center gap-6 text-sm text-white/90 md:flex">
            <NavigationMenu>
              <NavigationMenuList>
                <NavigationMenuItem>
                  <NavigationMenuTrigger
                    className="bg-transparent text-white/90 hover:text-[#ff364d] data-[state=open]:text-[#ff364d]
                          ...
                          <NavigationMenuLink asChild>
                            <Link
                              ...
                              className="group relative flex items-start gap-3 rounded-xl p-3 transition-all
                                         hover:bg-white/5 hover:ring-1 hover:ring-[#ff364d]/60
                                         hover:shadow-[0_0_0_1px_rgba(255,54,77,0.25),0_0_20px_rgba(255,54,77,0.15)]
                                         focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ff364d]/70"
                            >
                              <service.icon className="h-5 w-5 text-[#ff364d] mt-0.5 shrink-0 group-hover:text-[#ff364d]" />
                              <div>
                                <div className="text-sm font-medium text-white group-hover:text-[#ff364d]">
                                  {service.label}
                                </div>
            ...
            {links.map((l) => (
              <Link key={l.href} href={l.href} className="hover:text-[#ff364d] transition-colors">
                {l.label}
              </Link>
            ))}
          </nav>
              ...
              <SheetContent side="right" className="liquid-glass border-gray-800 p-0 w-64 flex flex-col">
                ...
                <nav className="flex flex-col gap-1 mt-2 text-gray-200">
                  <Collapsible open={servicesOpen} onOpenChange={setServicesOpen}>
                    <CollapsibleTrigger className="flex items-center justify-between w-full px-4 py-3 hover:bg-gray-900 hover:text-[#ff364d] transition-colors">
                    ...
                    <CollapsibleContent>
                      <div className="flex flex-col bg-gray-900/50 border-l-2 border-[#ff364d]/30 ml-4">
                        {services.map((service) => (
                          <Link
                            key={service.href}
                            href={service.href}
                            className="flex items-center gap-3 pl-8 pr-4 py-2.5 hover:bg-gray-900 hover:text-[#ff364d] transition-colors"
                          >
                            <service.icon className="h-4 w-4 text-[#ff364d]/70" />
                  ...
                  {links.map((l) => (
                    <Link
                      key={l.href}
                      href={l.href}
                      className="flex items-center gap-3 px-4 py-3 hover:bg-gray-900 hover:text-[#ff364d] transition-colors"
                    >
                      <span className="inline-flex items-center justify-center w-5 h-5 text-gray-400">
